...
\begin{itemize}[leftmargin=*,topsep=2pt,itemsep=1pt]
  \item \textbf{Network (nw)}: \texttt{tc netem} delay, range 0 to 10\,s.
  \item \textbf{Filesystem (fs)}: Simulated I/O delay, range 0 to 10\,s.
  \item \textbf{CPU (cpu)}: cgroup throttling, range 0.1 to 2.0 cores.
  \item \textbf{Memory (mem)}: cgroup limit, range 128\,MB to 4\,GB.
...
\begin{itemize}[leftmargin=*,topsep=2pt,itemsep=1pt]
  ...
  \item \textbf{Bayesian (GP-UCB)}: Uncertainty-guided sampling that targets the
...
\textbf{Bold} indicates best result per system. ``N/F'' indicates boundary not found within budget.
...
Redis 7.2          & 37 & 1064\,s & N/F         & 14.3$\pm$5.2 & 410\,s  & 4.8\,s   & \textbf{17} & \textbf{488\,s}  & 2.7\,s \\
...
\textbf{Speedup}   & 1.0$\times$ & & & N/F & & & \textbf{2.1$\times$} & \textbf{2.1$\times$} & \\
\bottomrule
\end{tabular}
\end{table*}

\para{Trial efficiency} \system{} uses 179 trials vs.\ \xinda{}'s 370 across
10 systems (51.6\% reduction). Each system requires exactly 17 to 18 minimizer
...
estimates are 10 to 100$\times$ higher than \system{}'s minimized values,
...
\midrule
\multicolumn{6}{@{}l}{\textit{Iterations per minimization: 17 to 18 (nw/fs), 8 to 10 (cpu/mem), 1 (process)}} \\
\bottomrule
\end{tabular}
\end{table*}

Network boundaries span 130$\times$ from Raft systems (19.5\,ms) to Redis
(2.7\,s). Filesystem boundaries are 4 to 16$\times$ higher (synchronous WAL
...
22 to 50\% (mean 31.4\%), demonstrating interaction effects invisible to
single-fault testing. Single-fault boundaries are \emph{optimistic}; the
true danger zone under combined faults is 20 to 50\% tighter.
...
fewer probes. Bayesian optimization converges within 1 to 2 steps of the true
