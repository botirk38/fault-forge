...
\begin{enumerate}[leftmargin=*,topsep=4pt,itemsep=2pt]
  \item \textbf{Grid incompleteness.} Systems whose vulnerability boundaries
        ...
        example, requires 2.7\,s of sustained delay to trigger PFAIL, which is
        entirely outside \xinda{}'s 100\,$\mu$s--1\,s range. Similarly, filesystem
...
is both human-interpretable and CI-actionable. While binary search itself is
standard, applying it effectively to distributed system fault testing requires
addressing several practical challenges: validating the monotonicity assumption
empirically (non-determinism in real systems introduces noise), decomposing a
multi-dimensional parameter space into independent searchable dimensions, and
integrating the search into a full lifecycle pipeline (container orchestration,
fault injection, log collection, oracle evaluation) that must execute reliably
...
\begin{itemize}[leftmargin=*,topsep=4pt,itemsep=2pt]
  ...
  \item Consensus-based systems (etcd, ZooKeeper, MongoDB, TiKV, CockroachDB)
        ...
        \textbf{sub-20\,ms} network delay, which is 250$\times$ below documented
