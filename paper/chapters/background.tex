...
congestion, switch buffers, or NIC issues. Even microsecond-scale delays can
disrupt heartbeat-based failure detectors~\cite{raft,zab}. Cloud providers
report tail latency spikes of 1 to 10\,ms multiple times per day on inter-AZ
links~\cite{dean-tail-latency}.

\para{Filesystem slow faults} Increased I/O latency from disk degradation or
journaling contention, particularly affecting WAL performance.
Storage-related performance bugs account for 25\% of cloud system
issues~\cite{lu-cloud-reliability}.

\para{CPU slow faults} Resource throttling from cgroup limits or noisy
neighbors that starve critical threads. Kubernetes CPU throttling affects
over 60\% of production containers~\cite{datadog-monitoring}.

\para{Memory pressure faults} Reduced memory forcing GC pauses in JVM-based
systems, page cache eviction, or swap thrashing, creating cascading slowdowns.

\para{Process faults} Container restarts causing temporary unavailability,
where the \emph{timing} relative to in-flight operations determines impact.
...
efficient for systems with boundaries in the 1 to 100\,ms range but wasteful for
...
a leader election, then 25\,ms, 50\,ms, and 100\,ms also cause it because the
higher delay strictly subsumes the lower. This \emph{monotonicity property} enables
