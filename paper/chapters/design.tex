...
iterations. Empirically we observe 17 to 18 (the initial verification trial
...
\node[blue, font=\scriptsize, below] at (8,0.3) {etcd};
...
\node[red, font=\scriptsize, right] at (8.2,5.1) {Redis};
...
\node[green!60!black, font=\scriptsize, above] at (8,1.1) {Kafka};
...
\node[gray, font=\scriptsize, below] at (4,1.2) {$S_{\max}/2^k$};
...
Before minimization, \system{} needs a reproducing trial. In practice,
operators know a high-severity configuration that reproduces
(\eg{}, ``5\,s delay always triggers election''). The minimizer starts from
this known-good trial, eliminating an initial search phase in most cases.

\subsection{Multi-Fault Minimization}
\label{sec:multi-fault}

For trials with multiple faults, Phase~1 attempts to eliminate each fault
while preserving reproduction, identifying \emph{necessary} faults. The
greedy approach preserves interaction effects: it only removes a fault if the
remaining set still reproduces. Phases 2 to 4 then minimize each remaining
fault independently, producing a locally minimal multi-fault recipe.
