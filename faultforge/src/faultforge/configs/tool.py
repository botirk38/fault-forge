import os
from faultforge.configs.reslim import *
...
class Tool:
    def __init__(
        ...
        self.install_root = install_root_
        self.tooling_root = tooling_root_
        self.charybdefs_mount_dir = charybdefs_mount_dir_
        self.reslim = reslim_
        # Scripts
        self.jacoco = os.path.join(tooling_root_, ("docker-" + sys_name_), "jacoco")
        if version_ != None:
            self.compose = os.path.join(tooling_root_, ("docker-" + sys_name_), version_)
        else:
            self.compose = os.path.join(tooling_root_, ("docker-" + sys_name_))
        ...
        if not os.path.exists(self.compose):
            raise FileNotFoundError(
                f"Compose folder {self.compose} does not exist, check your version and coverage arguments"
            )

        self.blockade = os.path.join(tooling_root_, "blockade")
        # Tools/Softwares
        self.ycsb = os.path.join(install_root_, "ycsb-0.17.0")
        self.go_ycsb = os.path.join(install_root_, "go-ycsb-source")
        self.go_ycsb_bin = os.path.join(install_root_, "go-ycsb")
        self.cfs_source = os.path.join(install_root_, "charybdefs")
        self.ycsb_wkl_root = os.path.join(install_root_, "ycsb-workloads")
        ...
        # Cassandra
        self.cas_cqlsh = os.path.join(install_root_, "cas/bin/cqlsh")
        self.cas_init_cql = os.path.join(tooling_root_, "init.cql")

        # HBase
        self.hbase_init = os.path.join(tooling_root_, "hbase-init.sh")
        self.hbase_conf = os.path.join(tooling_root_, "hbase-site.xml")
        if change_workload_ and sys_name_ == "hbase":
            self.hbase_init = os.path.join(tooling_root_, "hbase-init2.sh")
            print("Copying hbase-init2.sh")
        self.hbase_check_pid = os.path.join(tooling_root_, "hbase-check-pid.sh")
        self.hbase_ycsb = "/tmp/ycsb-0.17.0"
        self.hbase_ycsb_wkl = "/tmp/ycsb-workloads/ycsb"

        # MapReduce
        self.hadoop_mapreduce_client_local = os.path.join(
            install_root_,
            f"hadoop-{version_}/hadoop-mapreduce-project/hadoop-mapreduce-client/hadoop-mapreduce-client-jobclient/target",
        )
        ...
        self.hadoop_mapreduce_examples_local = os.path.join(
            install_root_,
            f"hadoop-{version_}/hadoop-mapreduce-project/hadoop-mapreduce-examples/target",
        )
        self.mapred_terasort_on_container = f"/opt/hadoop-{version_}/share/hadoop/mapreduce/hadoop-mapreduce-examples-{version_}.jar"

        # Kafka
        self.kafka_compiled_source = os.path.join(install_root_, "kafka")
        self.openmsg_compiled_source = os.path.join(install_root_, "openmessaging")
        self.generate_env()
