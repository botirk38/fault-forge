...
class OFFICIAL_ETCD(Benchmark):
    def __init__(
        ...
        if workload_ == "stm":
            self.identifier = (
                "official-" + workload_ + "-isolation_" + isolation_ + "-locker_" + stm_locker_
            )
        else:
        ...
        if workload_ in ["lease-keepalive", "range", "txn-put"]:
            ...
            self.official_flags = (
                f"--total {self.total} --isolation {isolation_} --stm-locker {stm_locker_}"
            )
        if workload_ == "range":
            workload_ = "range key"
        self.workload = workload_
...
class MRBENCH_MAPRED(Benchmark):
    def __init__(self, workload_="mrbench", num_reduces_="3", num_iter_=10):
        self.workload = workload_
