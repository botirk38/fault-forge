...
class Cassandra(TestSystem):
    ...
    def _docker_status_checker(self):
        ...
        while True:
            ...
            if num_normal_node == b"3":
                ...
                counter = counter + 10
                print(
                    f"Still waiting for cluster to set up. Sleep 10s / Elapsed {counter}s / Need ~210s)"
                )
                time.sleep(10)
            if counter > 600:
    ...
    def _load_ycsb(self):
        cmd = [
            os.path.join(self.tool.ycsb, "bin/ycsb.sh"),
            "load",
            "cassandra-cql",
            f"-p hosts={self.container_info['cas1']}",
            "-s",
            "-P",
            os.path.join(self.tool.ycsb_wkl, ("workload" + self.benchmark.workload)),
            f"-p recordcount={self.benchmark.recordcount}",
        ]
        cmd = [str(item) for item in cmd]
        p = subprocess.run(cmd)
        self.info(
            f"{self.tool.ycsb}/workloads/workload{self.benchmark.workload} successfully loaded"
        )
    ...
    def _run_ycsb(self):
        ...
        self.ycsb_process = subprocess.Popen(
            cmd, stderr=open(self.log.runtime, "w"), stdout=open(self.log.raw, "w")
        )
