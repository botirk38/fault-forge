...
class Copilot(TestSystem):
    ...
    def _run_copilot(self):
        ...
        while True:
            if self.check_if_benchmark_starts():
                ...
                print(
                    f"{sleep_time}s> copilot-{self.benchmark.scheme} not started yet. Sleep for 1s"
                )
                time.sleep(1)
                sleep_time += 1
            if sleep_time >= 30:
        ...
        self.info(
            f"Benchmark:copilot, scheme:{self.benchmark.scheme} starts.", rela=self.start_time
        )
    ...
    def _wait_till_benchmark_ends(self):
        ...
        try:
            ...
            self.info(
                f"The subprocess took too long (>={copilot_timeout}s) to complete and was killed.",
