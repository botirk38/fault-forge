...
class HBase(TestSystem):
    ...
    def _copy_file_to_container(self):
        cmd = ["docker", "cp", self.tool.hbase_init, f"{self.hbase_init_loc}:/tmp/hbase-init.sh"]
        p = subprocess.run(cmd)
        cmd = ["docker", "cp", self.tool.ycsb, f"{self.dest}:/tmp/"]
        p = subprocess.run(cmd)
        cmd = ["docker", "cp", self.tool.ycsb_wkl_root, f"{self.dest}:/tmp/"]
        p = subprocess.run(cmd)
        cmd = ["docker", "exec", "-it", self.dest, "mkdir", "/tmp/conf"]
        p = subprocess.run(cmd)
        cmd = ["docker", "cp", self.tool.hbase_conf, f"{self.dest}:/tmp/conf/hbase-site.xml"]
        p = subprocess.run(cmd)

    def _init_hbase(self):
        cmd = ["docker", "exec", "-it", self.hbase_init_loc, "bash", "/tmp/hbase-init.sh"]
        try:
    ...
    def _load_ycsb(self):
        ...
        try:
            ...
            self.info(
                f"{self.tool.ycsb}/workloads/workload{self.benchmark.workload}-{self.benchmark.columnfamily} successfully loaded"
            )
        except subprocess.TimeoutExpired:
            self.info(
                f"{self.tool.ycsb}/workloads/workload{self.benchmark.workload}-{self.benchmark.columnfamily} load timeout ({loadTimeout}s)"
            )
            self._post_process()
            self.docker_down()
            if self._primary_fault().fault_type == "nw":
    ...
    def _load_ycsb2(self):
        ...
        try:
            p = subprocess.run(cmd, shell=True, timeout=loadTimeout)
            self.info(
                f"{self.tool.ycsb}/workloads/workload{self.benchmark2.workload}-{self.benchmark2.columnfamily} successfully loaded"
            )
        except subprocess.TimeoutExpired:
            self.info(
                f"{self.tool.ycsb}/workloads/workload{self.benchmark2.workload}-{self.benchmark2.columnfamily} load timeout ({loadTimeout}s)"
            )
            self._post_process()
            self.docker_down()
            if self._primary_fault().fault_type == "nw":
    ...
    def _run_ycsb(self):
        ...
        cmd = " ".join(cmd)
        self.info(
            f"Benchmark:ycsb-{self.benchmark.workload} starts. We should inject faults after 30s till the cluster performance is stable",
            rela=self.start_time,
        )
        try:
            self.ycsb_process = subprocess.run(
                cmd, shell=True, timeout=2 * (int(self.benchmark.exec_time))
            )
        except subprocess.TimeoutExpired:
            self.info(
                f"ycsb_process took too long (>={2 * (int(self.benchmark.exec_time))}s) to complete and was killed.",
                rela=self.start_time,
            )
            self._post_process()
            self.docker_down()
    ...
    def _run_ycsb2(self):
        ...
        try:
            self.ycsb_process = subprocess.run(
                cmd, shell=True, timeout=2 * (int(self.benchmark2.exec_time))
            )
        except subprocess.TimeoutExpired:
            self.info(
                f"ycsb_process took too long (>={2 * (int(self.benchmark2.exec_time))}s) to complete and was killed.",
                rela=self.start_time,
            )
            self._post_process()
            self.docker_down()
