...
class Etcd(TestSystem):
    ...
    def test(self):
        self.info(self._primary_fault().get_info(), if_time=False)
        if self._primary_fault().fault_type == "nw":
            ...
            if self.cluster_size == 10:
                ...
                time.sleep(40)
            self.docker_get_status()
        elif self._primary_fault().fault_type == "none":
            self.docker_up()
            time.sleep(10)
            self.docker_get_status()
        else:
            raise ValueError(
                f"Fault type:{self._primary_fault().fault_type} is not one of {{nw, fs}}"
            )
        self.get_leader_name()
        if self.benchmark.benchmark == "ycsb":
            self._load_ycsb()
            self._run_ycsb()
            if self._primary_fault().fault_type != "none":
                self.inject(cfs_pattern=f".*{self._primary_fault().location}.*")
            else:
    ...
    def get_leader_name(self):
        ...
        awk_process = subprocess.Popen(
            awk_cmd, shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE
        )
        ...
        self.info(
            f"Leader: {self.leader_name}, followers:{etcd_nodes}, choose one follower: {self.follower_name}"
        )
        fault = self._primary_fault()
        fault_loc = fault.location
        if fault.location == "leader":
            fault.location = self.leader_name
        elif fault.location == "follower":
            fault.location = self.follower_name
        self.info(f"Fault injection location changed from {fault_loc} to {fault.location}")
    ...
    def _load_ycsb(self):
        ...
        cmd = [
            self.tool.go_ycsb_bin,
            "load",
            "etcd",
            "-P",
            os.path.join(self.tool.go_ycsb_wkl, ("workload" + self.benchmark.workload)),
            "-p",
            f"recordcount={self.benchmark.recordcount}",
            "-p",
            f"etcd.endpoints={self.ycsb_endpoints}",
            "-p",
            f"threadcount={self.benchmark.threadcount}",
        ]
        cmd = [str(item) for item in cmd]
        p = subprocess.run(cmd)
        self.info(
            f"{self.tool.go_ycsb}/workloads/workload{self.benchmark.workload} successfully loaded"
        )
    ...
    def _run_ycsb(self):
        cmd = [
            ...
            os.path.join(self.tool.go_ycsb_wkl, ("workload" + self.benchmark.workload)),
            "-p",
            f"operationcount={self.benchmark.operationcount}",
            "-p",
            f"etcd.endpoints={self.ycsb_endpoints}",
            "--interval",
            self.benchmark.status_interval,
            "-p",
            f"threadcount={self.benchmark.threadcount}",
        ]
        cmd = [str(item) for item in cmd]
        self.ycsb_process = subprocess.Popen(
            cmd,
            stdout=open(self.log.runtime, "w"),
            stderr=subprocess.DEVNULL,
        )
        self.start_time = int(time.time() * 1e9)
        self.info(
            "Benchmark:ycsb starts. We should inject faults after 30s till the cluster performance is stable",
    ...
    def _run_ycsb_double(self):
        ...
        cmd = [
            ...
            "etcd",
            "-P",
            os.path.join(self.tool.go_ycsb_wkl, ("workload" + wkl)),
            "-p",
            f"operationcount={self.benchmark.operationcount}",
            "-p",
            f"etcd.endpoints=http://{self.container_info['etcd0']}:2379",
            "--interval",
            self.benchmark.status_interval,
            "-p",
            f"threadcount={self.benchmark.threadcount}",
    ...
    def run_official(self):
        ...
        self.official_process = subprocess.Popen(
            cmd,
            stdout=open(self.log.runtime, "w"),
            stderr=subprocess.DEVNULL,
            shell=True,
        ...
        self.start_time = int(time.time() * 1e9)
        self.info(
            f"Benchmark:{self.benchmark.identifier} starts. We should inject faults after 30s till the cluster performance is stable",
            rela=self.start_time,
        )
    ...
    def _wait_till_official_ends(self):
        cur_time = self.get_current_ts()
        self.info(
            f"Waiting till benchmark ends. Current time:{cur_time}, max_execution_time:{self.benchmark.max_execution_time}",
            rela=self.start_time,
        )
        ...
        if self.official_process.returncode == 0:
            ...
            self.info(
                f"[FATAL ERROR] Benchmark returncode={self.official_process.returncode} (not zero)!",
                rela=self.start_time,
            )
    ...
    def _post_process(self):
        p = subprocess.run(
            ["docker-compose", "logs"],
            stdout=open(self.log.compose, "w"),
        ...
        self.info(
            f"Current leader: {cur_leader_name}; Previous leader: {self.leader_name}; Leader changed: {cur_leader_name != self.leader_name}"
        )
