    ...
    def _run_depfast(self):
        ...
        while True:
            if self.check_if_benchmark_starts():
                ...
                print(
                    f"{sleep_time}s> depfast-{self.benchmark.scheme} not started yet. Sleep for 1s"
                )
                time.sleep(1)
                sleep_time += 1
            if sleep_time >= 30:
                raise Exception(
                    f"scheme:{self.benchmark.scheme} never starts after {self.sleep_time}s timeout"
                )
        self.start_time = int(time.time() * 1e9)
        self.info(
            f"Benchmark:depfast, scheme:{self.benchmark.scheme} starts.", rela=self.start_time
        )

    def _wait_till_benchmark_ends(self):
        depfast_timeout = (
            int(self.benchmark.exec_time)
            - int((int(time.time() * 1e9) - self.start_time) / 1e9)
            + 90
        )
        ...
        try:
            ...
            self.info(
                f"The subprocess took too long (>={depfast_timeout}s) to complete and was killed.",
