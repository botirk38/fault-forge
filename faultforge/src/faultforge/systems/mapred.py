...
class Mapred(TestSystem):
    ...
    def _mrbench_run(self):
        ...
        for i in range(1, self.benchmark.num_iter + 1):
            raw_log = os.path.join(
                self.log.data_dir,
                "raw-"
                + self._primary_fault().location
                + "-"
                + self._primary_fault().info
                + "-"
                + self.log.iter
                + "-mrbench"
                + ".log",
            ...
            while not check_mrbench_completion(runtime_log):
                ...
                _ = subprocess.run(
                    cmd, shell=True, stdout=open(raw_log, "a"), stderr=open(runtime_log, "a")
                )
    ...
    def _jacoco_get_report(self):
        if self.version == "3.3.6":
            ...
            file_paths = [
                "/opt/hadoop-3.0.0/share/hadoop/common/lib/hamcrest-core-1.3.jar",
