...
class Crdb(TestSystem):
    def test(self):
        ...
        if self._primary_fault().fault_type == "nw":
            ...
            raise ValueError(
                f"Fault type:{self._primary_fault().fault_type} is not one of {{nw, fs, none}}"
            )
    ...
    def _run_ycsb(self):
        ...
        self.benchmark_process = subprocess.Popen(
            cmd, shell=True, stdout=open(self.log.runtime, "w")
        )
        self.start_time = int(time.time() * 1e9)
        self.info(
            "Benchmark:ycsb starts. We should consider injecting faults after 30s till the cluster performance is stable",
    ...
    def _post_process(self):
        log_dict = [
            {
                "log_on_container": "/cockroach/cockroach-data/logs/cockroach.log",
                "log_on_host": self.log.crdb_log,
            },
            {
                "log_on_container": "/cockroach/cockroach-data/logs/cockroach-pebble.log",
                "log_on_host": self.log.crdb_pebble_log,
            },
            {
                "log_on_container": "/cockroach/cockroach-data/logs/cockroach-health.log",
                "log_on_host": self.log.crdb_health_log,
            },
            {
                "log_on_container": "/cockroach/cockroach-data/logs/cockroach-stderr.log",
                "log_on_host": self.log.crdb_stderr_log,
            },
        ]
        for log in log_dict:
            get_symlink_cmd = (
                f"docker exec {self._primary_fault().location} readlink {log['log_on_container']}"
            )
            ...
            get_reallog_cmd = (
                f"docker cp {self._primary_fault().location}:{symlink} {log['log_on_host']}"
            )
    ...
    def _sysbench_load(self):
        cmd = [
            "docker run --rm -it --network=docker-crdb_roachnet rmlu/sysbench:latest",
            "src/sysbench",
    ...
    def _sysbench_run(self):
        ...
        cmd = " ".join(cmd)
        self.benchmark_process = subprocess.Popen(
            cmd, shell=True, stdout=open(self.log.runtime, "a")
        )
        self.start_time = int(time.time() * 1e9)
