...
from faultforge.systems.TestSystem import *
...
class Kafka(TestSystem):
    def test(self):
        ...
        if self.benchmark.benchmark == "perf_test":
            ...
            raise ValueError(
                f"Benchmark: {self.benchmark.workload} is not one of {{perf_test, openmsg}}"
            )
        self._post_process()
        self.docker_down()
    ...
    def init_kafka(self):
        ...
        cmd = [
            "docker exec -it kafka-benchmarking",
    ...
    def basic_producer(self):
        cmd = [
            "docker exec kafka-benchmarking",
        ...
        self.producer_process = subprocess.Popen(
            cmd, shell=True, stdout=open(self.log.kafka_producer, "a"), stderr=subprocess.DEVNULL
        )
    ...
    def basic_consumer(self):
        ...
        self.consumer_process = subprocess.Popen(
            cmd, shell=True, stdout=open(self.log.kafka_consumer, "a"), stderr=subprocess.DEVNULL
        )
    ...
    def _wait_till_perftest_ends(self):
        ...
        if cur_time < self.benchmark.exec_time and self.producer_process.returncode != 0:
            self.info(
                f"Sleep {self.benchmark.exec_time - cur_time}s till the end", rela=self.start_time
            )
    ...
    def _wait_till_openmsg_ends(self):
        ...
        counter = 0
        while True:
            if self.check_string_in_file(
                file_path=self.log.openmsg_driver, target_string="Writing test result"
            ):
            ...
            cur_time = self.get_current_ts()
            self.info(
                f"Waiting for benchmark to end. Sleep 10s / Elapsed {counter}s / Need at most ~{self.benchmark.exec_time - cur_time}s)",
                rela=self.start_time,
            )
            if counter >= self.benchmark.exec_time + 300:
                self.info(
                    f"FATAL: benchmark does not end even after {self.benchmark.exec_time}+300={self.benchmark.exec_time + 30}s"
                )
                exit(1)
        ...
        if self.is_port_in_use([8082, 8083, 8084, 8085]):
            self.info(
                "Kill all processes running on port 8082,8083,8084,8085", rela=self.start_time
            )
    ...
    def _openmsg_change_test_duration(self):
        ...
        with open(
            os.path.join(
                self.tool.openmsg_compiled_source, f"workloads/{self.benchmark.workload_file}.yaml"
            ),
            "r",
        ) as file:
        ...
        self.info(
            f"Execution time will be rounded: {self.benchmark.exec_time}s -> {new_exec_time}min*60={new_exec_time * 60}s"
        )
        self.benchmark.change_exec_time(new_exec_time * 60)
        data["testDurationMinutes"] = new_exec_time
        with open(
            os.path.join(self.tool.openmsg_compiled_source, "workloads/this.yaml"), "w"
        ) as file:
    ...
    def _openmsg_run_driver(self):
        ...
        self.driver_process = subprocess.Popen(
            "exec " + cmd,
            shell=True,
            stdout=open(self.log.openmsg_driver, "a"),
            cwd=self.tool.openmsg_compiled_source,
        )

    def _openmsg_run_worker1(self):
        cmd = ["bin/benchmark-worker", "--port 8082", "--stats-port 8083"]
        cmd = " ".join(cmd)
        self.worker1_process = subprocess.Popen(
            "exec " + cmd,
            shell=True,
            stdout=open(self.log.openmsg_worker1, "a"),
            cwd=self.tool.openmsg_compiled_source,
        )

    def _openmsg_run_worker2(self):
        cmd = ["bin/benchmark-worker", "--port 8084", "--stats-port 8085"]
        cmd = " ".join(cmd)
        self.worker2_process = subprocess.Popen(
            "exec " + cmd,
            shell=True,
            stdout=open(self.log.openmsg_worker2, "a"),
            cwd=self.tool.openmsg_compiled_source,
        )
    ...
    def _openmsg_check_if_warmup_ends(self):
        ...
        while True:
            if self.check_string_in_file(
                file_path=self.log.openmsg_driver, target_string="Starting benchmark traffic"
            ):
    ...
    def _post_process(self):
        ...
        p = subprocess.run(
            f"mv *.json {self.log.openmsg_summary}",
            shell=True,
            stdout=open(self.log.openmsg_worker1, "a"),
            cwd=self.tool.openmsg_compiled_source,
        )
